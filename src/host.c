...
fields_t *host_setup(int Nx, int Ny, int Nz,
                     ...
                     unsigned int seed, bool generate) {
  fields_t *gh_fields;
      ...
      for(int z=0; z < Nz+1; ++z){
        if (y < Ny && z < Nz) {
          if (!is_equal_f(
                ...
                field_2->Hx[x + y*(Nx+1) + z*Ny*(Nx+1)], tolerance)){
        ...
        if (x<Nx && z < Nz){
          if (!is_equal_f(
                ...
                field_2->Hy[x + y*Nx + z*(Ny+1)*Nx], tolerance)){
      ...
      for(int z=0; z < Nz + 1; ++z){
        if (x < Nx){
          if (!is_equal_f(
                ...
                field_2->Ex[x + y*Nx + z*(Ny+1)*Nx], tolerance)){
        ...
        if (y < Ny){
          if (!is_equal_f(
                ...
                field_2->Ey[x + y*(Nx+1) + z*Ny*(Nx+1)], tolerance)){
        ...
        if (z < Nz){
          if (!is_equal_f(
                ...
                field_2->Ez[x + y*(Nx+1) + z*(Ny+1)*(Nx+1)], tolerance)){
...
void update_fields(fields_t *h_fields) {
  ...
  for(int x=0; x < Nx+1; ++x){
    for(int y=0; y < Ny+1; ++y){
      for(int z=0; z < Nz+1; ++z){
        if (y < Ny && z < Nz) {
          Hx[x + y*(Nx+1) + z*Ny*(Nx+1)] += (dt/MU0) * (
            (Ey[x + y*(Nx+1) + (z+1)*Ny*(Nx+1)] -Ey[x + y*(Nx+1) + z*Ny*(Nx+1)])/dz -
            (Ez[x + (y+1)*(Nx+1) + z*(Ny+1)*(Nx+1)]-Ez[x + y*(Nx+1) + z*(Ny+1)*(Nx+1)])/dy);
        }

        if (x<Nx && z < Nz){
          Hy[x + y*Nx + z*(Ny+1)*Nx] += (dt/MU0) * (
            (Ez[(x+1) + y*(Nx+1) + z*(Ny+1)*(Nx+1)]-Ez[x + y*(Nx+1) + z*(Ny+1)*(Nx+1)])/dx -
            (Ex[x + y*Nx + (z+1)*(Ny+1)*Nx]-Ex[x + y*Nx + z*(Ny+1)*Nx])/dz);
        }

        if (x<Nx && y < Ny){
          Hz[x + y*Nx + z*Ny*Nx] += (dt/MU0) * (
            (Ex[x + (y+1)*Nx + z*(Ny+1)*Nx]-Ex[x + y*Nx + z*(Ny+1)*Nx])/dy -
            (Ey[(x+1) + y*(Nx+1) + z*Ny*(Nx+1)]-Ey[x + y*(Nx+1) + z*Ny*(Nx+1)])/dx);
        }
      }
    }
  }
  ...
  for(int x=0; x < Nx; ++x){
    for(int y=0; y < Ny; ++y){
      for(int z=0; z < Nz; ++z){
        if (y>0 && z>0){
          Ex[x + y*Nx + z*(Ny+1)*Nx] += (dt/EPS0) * (
              (Hz[x+y*Nx+z*Ny*Nx]-Hz[x+(y-1)*Nx+z*Ny*Nx])/dy -
              (Hy[x+y*Nx+z*(Ny+1)*Nx]-Hy[x+y*Nx+(z-1)*(Ny+1)*Nx])/dz);
        }
        if(x>0 && z>0){
          Ey[x + y*(Nx+1) + z*Ny*(Nx+1)] += (dt/EPS0) * (
              (Hx[x+y*(Nx+1)+z*Ny*(Nx+1)]-Hx[x+y*(Nx+1)+(z-1)*Ny*(Nx+1)])/dz -
              (Hz[x+y*Nx+z*Ny*Nx]-Hz[(x-1)+y*Nx+z*Ny*Nx])/dx);
        }
        if(x>0 && y>0){
          Ez[x + y*(Nx+1) + z*(Ny+1)*(Nx+1)] += (dt/EPS0) * (
              (Hy[x+y*Nx+z*(Ny+1)*Nx]-Hy[(x-1)+y*Nx+z*(Ny+1)*Nx])/dx -
              (Hx[x+y*(Nx+1)+z*Ny*(Nx+1)]-Hx[x+(y-1)*(Nx+1)+z*Ny*(Nx+1)])/dy);
        }
      }
    }
  }
    ...
    if (sample_freq > 0){
      if (i%sample_freq == 0){
        ...
        for (int i = 0; i < NR_SAMPLES; ++i){
          ...
          z = sample_loc[i][2];
          printf(",%e,%e,%e",
                  h_fields->Ex[x+y*h_fields->Nx+z*(h_fields->Ny+1)*h_fields->Nx],
                  h_fields->Ey[x+y*(h_fields->Nx+1)+z*h_fields->Ny*(h_fields->Nx+1)],
                  h_fields->Ez[x+y*(h_fields->Nx+1)+z*(h_fields->Ny+1)*(h_fields->Nx+1)]);
